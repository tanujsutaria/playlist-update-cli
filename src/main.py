...
# Maintenance vertical (PR 6): thin delegates below call these bodies.
from commands import maintenance, track_ops

# Command registry — extracted to commands/dispatch.py (PR 4 of the
# decomposition). Re-exported because interactive_app and the tests import
# them from main; _COMMAND_HANDLERS must remain the SAME dict object in both
# modules (tests mutate it via monkeypatch.setitem) — pinned by
# tests/test_registry_parser_sync.py.
from commands.debug import (  # noqa: F401
    _present_debug,
    _present_debug_last_search,
    _present_debug_track,
)
from commands.dispatch import _COMMAND_HANDLERS, dispatch_command  # noqa: F401
...
# Playlist-name resolver — extracted to playlist_resolver.py (PR 5). main's
# command methods call the miss/warn pair; the full quintet is re-exported
# for back-compat with older imports.
from playlist_resolver import (  # noqa: F401
...
class PlaylistCLI:
    ...
    def export_stats(
        ...
        return maintenance.export_stats(self, playlist_name, export_format, output_file)

    def sync_playlist(self, playlist_name: str):
        return maintenance.sync_playlist(self, playlist_name)

    def extract_playlist(self, playlist_name: str, output_file: Optional[str] = None):
        return maintenance.extract_playlist(self, playlist_name, output_file)

    def backup_data(self, backup_name: Optional[str] = None):
        return maintenance.backup_data(self, backup_name)

    def restore_data(self, backup_name: str) -> bool:
        return maintenance.restore_data(self, backup_name)

    def list_backups(self):
        return maintenance.list_backups(self)

    def clean_database(self, dry_run: bool = False):
        return maintenance.clean_database(self, dry_run)
    ...
    def _snapshot_playlist(self, playlist_name: str) -> List[Dict[str, Any]]:
        return track_ops._snapshot_playlist(self, playlist_name)

    def _record_undo(self, playlist_name: str, tracks: List[Dict[str, Any]]) -> None:
        return track_ops._record_undo(self, playlist_name, tracks)

    def undo_last_write(self) -> bool:
        return track_ops.undo_last_write(self)
    ...
    def _mirror_playlist_id(self, playlist_name: str) -> Optional[str]:
        return track_ops._mirror_playlist_id(self, playlist_name)

    def _track_candidate(self, track_id: str) -> Optional[Dict[str, Any]]:
        return track_ops._track_candidate(self, track_id)

    def _track_candidates(self, source_playlist: Optional[str] = None) -> List[Dict[str, Any]]:
        return track_ops._track_candidates(self, source_playlist)
    ...
    def _resolve_track(
        ...
        return track_ops._resolve_track(
            self, query, track_id=track_id, source_playlist=source_playlist
        )

    def _lives_in_line(self, track_id: str) -> str:
        return track_ops._lives_in_line(self, track_id)

    @staticmethod
    def _stored_spotify_uri(raw: Optional[str]) -> Optional[str]:
        return track_ops._stored_spotify_uri(raw)

    def _song_from_candidate(self, candidate: Dict[str, Any]) -> Song:
        return track_ops._song_from_candidate(self, candidate)

    def _remove_uri_for(self, candidate: Dict[str, Any]) -> Optional[str]:
        return track_ops._remove_uri_for(self, candidate)

    def _patch_mirror_membership(self, playlist_name: str, track_id: str, present: bool) -> None:
        return track_ops._patch_mirror_membership(self, playlist_name, track_id, present)

    def add_track_to_playlist(
        self, query: str, playlist_name: str, track_id: Optional[str] = None
    ) -> bool:
        return track_ops.add_track_to_playlist(self, query, playlist_name, track_id=track_id)

    def remove_track_from_playlist(
        self, query: str, playlist_name: str, track_id: Optional[str] = None
    ) -> bool:
        return track_ops.remove_track_from_playlist(self, query, playlist_name, track_id=track_id)
    ...
    def move_track(
        ...
        return track_ops.move_track(self, query, from_playlist, to_playlist, track_id=track_id)
